...
$totalpages =  ceil(num_rows(Sql_query("SELECT 1 FROM `folders`"))/rowsperpage);    
...
// find out total pages
$totalpages =  ceil(num_rows(Sql_query("SELECT DISTINCT `file_id` FROM `stats`"))/rowsperpage);    
...
if ($result=Sql_query("SELECT DISTINCT `file_id` FROM `stats`"))
  ...
  {
		  $html.= '<tr>
				<td>'.GetCountryName($_row["country_code"]).'</td>
				<td>'.$_row["browser"].'</td>
				<td>'.$_row["platform"].'</td>
...
// find out total pages
$totalpages =  ceil(num_rows(Sql_query("SELECT 1 FROM `reports`"))/rowsperpage);    
...
// find out total pages
$totalpages =  ceil(Sql_Get_Users_Count()/rowsperpage);    
  ...
  while($row = mysqli_fetch_assoc($result))
  {
	  ...
	  if(file_exists('../..'.$folder.'/'.$row["filename"]))
		  ...
		  $html.='<td><input value="'.$_file_id.'" type="checkbox"  name="files[]" class="checkbox" onclick="calcItems()"  /></td>
		        ...
		        <td><a data-numrows="'.$num_rows.'" id="showfile_'.$count.'" href="javascript:void(0)" onclick="ShowFileInfoModal('.$_file_id.','.$_crypt_id.','.$count .')">'.icon($row["filename"]).' '.$row["filename"].'</a></td>
...
if($total==0)
	$html = '<tr><td colspan="8">'.$lang[59].'(0)</td></tr>';
...
if(isset($_GET['delete_selected']))
{
	...
	$result=array();		
     foreach ($_POST['folders'] as $id) {
			unlinkRecursive('../..'.Get_folderName_By_FolderId($id), true);
				if(Sql_Delete_Folder($id))$result[]=  $id ; 
			}
...
if(isset($_GET['delete_folder']))
{
	...
	if(Sql_Delete_Folder($_GET['folder_id']))
		if(unlinkRecursive($_GET['delete_folder'], true))
			PrintArray(array('success_msg' => $lang[178]));
	else 
