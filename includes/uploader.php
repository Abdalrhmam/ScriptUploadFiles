...
class FileUpload {
    ...
    private function errorCodeToMsg($code) {
        switch($code) {
            case UPLOAD_ERR_INI_SIZE:
                $message = $this->Language[117];
                break;
            case UPLOAD_ERR_PARTIAL:
                $message = $this->Language[126];
                break;
            case UPLOAD_ERR_NO_FILE:
                $message = $this->Language[125];
                break;
            case UPLOAD_ERR_NO_TMP_DIR:
                $message = $this->Language[124];
                break;
            case UPLOAD_ERR_CANT_WRITE:
                $message = $this->Language[123];
                break;
            case UPLOAD_ERR_EXTENSION:
                $message = $this->Language[122];
                break;
            default:
                $message = $this->Language[121];
                break;
        }
        return $message;
    }
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null) {
        if (!$this->fileName) {
            $this->setErrorMsg($this->Language[115]);
            return false;
        }

        if ($this->fileSize == 0) {
            $this->setErrorMsg($this->Language[116]);
            return false;
        }

        if ($this->fileSize > $this->sizeLimit) {
            $this->setErrorMsg($this->Language[117]);
            return false;
        }
        ...
        if (!file_exists($this->uploadDir)) {
            $this->setErrorMsg($this->Language[118]);
            return false;

        } else if (!is_writable($this->uploadDir)) {
            $this->setErrorMsg($this->Language[119]);
            return false;
        }
        ...
        if (!empty($this->allowedExtensions)) {
            if (!$this->checkExtension($this->fileExtension, $this->allowedExtensions)) {
                $this->setErrorMsg($this->Language[120]);
                return false;
            }
        }
        ...
        if (!$this->save($this->savedFile)) {
            $this->setErrorMsg($this->Language[127]);
            return false;
        }
